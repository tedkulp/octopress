    ...
    def write_tag_indexes
      if self.layouts.key? 'tag_index'
        ...
        self.tags.keys.each do |tag|
          self.write_tag_index(File.join(dir, tag.gsub(/ä/i, 'ae').gsub(/ö/i, 'oe').gsub(/ü/i, 'ue').gsub(/_|\W/, '-')), tag)
        end
  ...
  module Filters
    ...
    def tag_links(tags)
      ...
      tags = tags.sort!.map do |item|
        "<a class='tag' href='/#{dir}/#{item.gsub(/ä/i, 'ae').gsub(/ö/i, 'oe').gsub(/ü/i, 'ue').gsub(/_|\W/, '-')}/'>#{item}</a>"
      end
